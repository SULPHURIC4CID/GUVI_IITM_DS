...
def authenticate(email,password):
    '''
    Authentication function, that returns true if authenticated and false if the credentials do not match the data in file.
    '''
...
def retrieve_password(email):
    '''
    Function that retrieves the old password from the file for the user.
    '''
    file_handle = open(file_name,'r');
    for line in file_handle:
        data = re.split("\s",line);
...
def change_password(email):
    '''
    Function allows the user to change his old password to a new password.
    '''
    ...
    while(True):
        ...
        if registration_module.check_password(new_password) == True:    #Check the validity of the password.
            break;
        else:
            print("\nEnter a valid password.");
            registration_module.password_requirements();    #Display the password requirements, if password is invalid.
...
def login():
    ...
    while(True):
        ...
        if (registration_module.check_email(user_email)==True):         #Checking if the user email is a valid email address.
            if registration_module.check_email_in_file(user_email) == True: #Checking if the email entered is present in the file.
                break;
            else:
